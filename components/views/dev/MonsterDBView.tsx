...
const MonsterDBView: React.FC<MonsterDBViewProps> = ({ monsters, setMonsters, modifiedMonsters, setModifiedMonsters, onClose, setTooltip, onCommit, hasChanges }) => {
    ...
    return (
        <div className="fixed inset-0 bg-gray-900/95 z-50 p-4 flex flex-col text-white animate-fade-in" onClick={onClose}>
                    ...
                    {!selectedMonster && (
                        <div>
                            ...
                            <div className="grid grid-cols-8 md:grid-cols-12 lg:grid-cols-16 gap-2">
                                ...
                                <div className="aspect-square p-2 bg-gray-900/50 rounded-md flex items-center justify-center border-2 border-dashed border-gray-600 cursor-pointer hover:bg-green-800/50 hover:border-green-500" title="Add New Monster">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div className="bg-gray-900/50 p-3 rounded-lg space-y-2">
                                    ...
                                    <div className="flex justify-between items-center text-sm">
                                        <label>Level:</label><input type="number" value={selectedMonster.level} onChange={e => updateMonster(selectedMonster.id, { level: +e.target.value })} className="w-20 p-1 bg-gray-800 border border-gray-700 rounded text-right" />
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <label>Max HP:</label><input type="number" value={selectedMonster.maxHp} onChange={e => updateMonster(selectedMonster.id, { maxHp: +e.target.value })} className="w-20 p-1 bg-gray-800 border border-gray-700 rounded text-right" />
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <label>Max Hit:</label><input type="number" value={selectedMonster.customMaxHit ?? ''} onChange={e => updateMonster(selectedMonster.id, { customMaxHit: e.target.value === '' ? undefined : +e.target.value })} placeholder="Auto" className="w-20 p-1 bg-gray-800 border border-gray-700 rounded text-right" />
                                    </div>
                                </div>
                                {/* Attack Stats */}
                                <div className="bg-gray-900/50 p-3 rounded-lg space-y-2">
                                    ...
                                    <div className="flex justify-between items-center text-sm">
                                        <label>Attack:</label><input type="number" value={selectedMonster.attack} onChange={e => updateMonster(selectedMonster.id, { attack: +e.target.value })} className="w-20 p-1 bg-gray-800 border border-gray-700 rounded text-right" />
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <label>Ranged:</label><input type="number" value={selectedMonster.ranged || 0} onChange={e => updateMonster(selectedMonster.id, { ranged: +e.target.value })} className="w-20 p-1 bg-gray-800 border border-gray-700 rounded text-right" />
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <label>Magic:</label><input type="number" value={selectedMonster.magic || 0} onChange={e => updateMonster(selectedMonster.id, { magic: +e.target.value })} className="w-20 p-1 bg-gray-800 border border-gray-700 rounded text-right" />
                                    </div>
                                </div>
                                {/* Defence Stats */}
                             ...
                             {/* Drop Tables */}
                            {['guaranteedDrops', 'mainDrops', 'tertiaryDrops'].map(tableType => (
                                <div key={tableType} className="bg-gray-900/50 p-3 rounded-lg">
                                    <h3 className="font-bold text-yellow-300 capitalize mb-2">{tableType.replace('Drops', ' Drops')}</h3>
                                    <div className="space-y-2">
                                        {(selectedMonster[tableType as keyof Monster] as any[] || []).map((drop, index) => {
                                            const isTableDrop = !!drop.tableId;
                                            const item = !isTableDrop && drop.itemId ? ITEMS[drop.itemId] : null;
                                            const isValid = isTableDrop || !!item;

                                            return (
                                                <div key={index} className="grid grid-cols-12 gap-2 items-center text-sm p-1 bg-gray-800/50 rounded">
                                                    <div className="col-span-3 flex items-center gap-2">
                                                        {isTableDrop ? (
                                                            <img src="https://api.iconify.design/game-icons:rolling-dices.svg" className="w-8 h-8 p-1 bg-black/30 rounded-md filter invert" alt="Loot Table" />
                                                        ) : item ? (
                                                            <img src={item.iconUrl} className={`w-8 h-8 p-1 bg-black/30 rounded-md ${getIconClassName(item)}`} alt={item.name} />
                                                        ) : (
                                                            <div className="w-8 h-8 bg-black/30 rounded-md flex items-center justify-center text-gray-500">?</div>
                                                        )}
                                                        <input 
                                                            type="text" 
                                                            value={drop.itemId || drop.tableId || ''} 
                                                            placeholder="itemId or tableId"
                                                            onChange={(e) => {
                                                                const value = e.target.value;
                                                                const isPotentiallyTable = !ITEMS[value];
                                                                if (isPotentiallyTable) {
                                                                    updateDrop(selectedMonster.id, tableType as any, index, { tableId: value, itemId: undefined });
                                                                } else {
                                                                    updateDrop(selectedMonster.id, tableType as any, index, { itemId: value, tableId: undefined });
                                                                }
                                                            }} 
                                                            className={`w-full p-1 bg-gray-900 border ${isValid ? 'border-gray-700' : 'border-red-500'} rounded`} 
                                                        />
                                                    </div>
                                                    <div className="col-span-2 flex items-center gap-1">Min: <input type="number" value={drop.minQuantity} onChange={(e) => updateDrop(selectedMonster.id, tableType as any, index, { minQuantity: +e.target.value })} className="w-16 p-1 bg-gray-900 border border-gray-700 rounded text-right" /></div>
                                                    <div className="col-span-2 flex items-center gap-1">Max: <input type="number" value={drop.maxQuantity} onChange={(e) => updateDrop(selectedMonster.id, tableType as any, index, { maxQuantity: +e.target.value })} className="w-16 p-1 bg-gray-900 border border-gray-700 rounded text-right" /></div>
                                                    <div className="col-span-2 flex items-center gap-1">Chance: <input type={tableType === 'tertiaryDrops' ? 'number' : 'text'} value={tableType !== 'guaranteedDrops' ? (drop as WeightedDrop).chance : 'ALWAYS'} disabled={tableType === 'guaranteedDrops'} onChange={(e) => updateDrop(selectedMonster.id, tableType as any, index, { chance: e.target.value })} className="w-20 p-1 bg-gray-900 border border-gray-700 rounded text-right disabled:opacity-50" /></div>
                                                    <div className="col-span-2 flex items-center justify-start gap-1" title="Drop as note">
                                                        <label htmlFor={`noted-check-${tableType}-${index}`} className="flex items-center gap-1 cursor-pointer select-none">
                                                            Noted:
                                                            <input
                                                                id={`noted-check-${tableType}-${index}`}
                                                                type="checkbox"
                                                                checked={!!drop.noted}
                                                                disabled={isTableDrop}
                                                                onChange={(e) => updateDrop(selectedMonster.id, tableType as any, index, { noted: e.target.checked })}
                                                                className="w-4 h-4 bg-gray-900 border-gray-700 rounded text-yellow-500 focus:ring-yellow-500 disabled:opacity-50 cursor-pointer"
                                                            />
                                                        </label>
                                                    </div>
                                                    <button onClick={() => handleRemoveDrop(selectedMonster.id, tableType as any, index)} className="col-span-1 text-red-500 hover:text-red-400 text-xl font-bold">×</button>
                                                </div>
                                            );
                                        })}
                                         <button onClick={() => handleAddDrop(selectedMonster.id, tableType as any)} className="w-full text-center py-2 bg-green-900/50 hover:bg-green-800/50 rounded-md border-2 border-dashed border-green-700/50">+</button>
                                    </div>
                                </div>
                            ))}
                        </div>
