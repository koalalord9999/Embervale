...
export const getMysticScore = (track: MusicTrackMetadata): string => {
    ...
    const style = { bpm: 90, scale: [NOTES.C4, NOTES.D4, NOTES.E4, NOTES.Fs4, NOTES.G4, NOTES.A4, NOTES.B4, NOTES.C5] };
    ...
    for (let t = 0; t < totalDuration; t += stepMs) {
        ...
        if (rng() < melodyChance && (step % 4 === 0 || (isPeak && step % 2 === 0))) {
            ...
            score += `${t}:osc:sine|freq:${note}|dur:0.6|vol:${leadVol}|filter:3000|attack:0.01|decay:0.8`;
        }
    }
    return score;
};
