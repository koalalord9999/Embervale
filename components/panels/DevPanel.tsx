...
interface DevPanelProps {
    ...
    devPanelState: {
        activeTab: 'cheats' | 'items' | 'teleport';
...
const CheatsComponent: React.FC<Omit<DevPanelProps, 'inv' | 'setTooltip' | 'onForcedNavigate' | 'devPanelState' | 'updateDevPanelState'>> = ({
...
const ItemSpawnerComponent: React.FC<{
    inv: ReturnType<typeof useInventory>;
    setTooltip: (tooltip: TooltipState | null) => void;
    searchTerm: string;
...
}> = ({ inv, setTooltip, searchTerm, setSearchTerm, selectedItem, setSelectedItem, quantity, setQuantity }) => {
...
const DevPanel: React.FC<DevPanelProps> = (props) => {
    const { inv, setTooltip, devPanelState, updateDevPanelState, ...otherProps } = props;
    const { activeTab, itemSearchTerm, selectedItemId, spawnQuantity, teleportRegionId, teleportPoiId } = devPanelState;

    const setActiveTab = (tab: 'cheats' | 'items' | 'teleport') => updateDevPanelState({ activeTab: tab });
